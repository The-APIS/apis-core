...
module.exports = ({
 ...
 const validateParamAddress = ({ key }) =>
 ...
 const validateQueryAddress = ({ key }) =>
  query(key).custom((value, { req }) => {
   if (!web3.utils.isAddress(value)) {
    throw new error(`invalid ${key}`);
   }
   return true;
  ...
  async (req, res, next) => {
   try {
    ...
    const collection = await Promise.all(
     Array.from(Array(Number(balance)).keys()).map((i) =>
  ...
  async (req, res, next) => {
   try {
    ...
    if (valueArray.length != idArray.length) {
     return res
      .status(500)
      .json({ error: "NO: OF ID's NOT EQUAL TO NO OF VALUES" });
    }
 ...
 router.post(
  ...
  async (req, res, next) => {
   try {
    ...
    const { address = "*" } = req.params;
    let { tokenAddress = "", uri = "", privateKey = "" } = { ...req.query };
    const tokenContractAddress = prefixContractAddress(
     tokenAddress.toString("hex")
    );
    const contractAbi = abi.APIS_ERC1155;
